...
// A pure dependency bump needs no hand-written `## [Unreleased]` bullet: the
// released sections of CHANGELOG.md are generated by semantic-release from
// conventional commits, so a `build(deps): …` commit is already recorded at
// release time. Without this exemption every Dependabot PR fails the policy —
// which is what trained the habit of merging those PRs red.
const DEPENDENCY_ONLY_PATHS = new Set(["package.json", "package-lock.json"]);
...
/**
 * True when `files` is a dependency bump and nothing else: it touches only
 * package.json / package-lock.json, and package.json differs from the base
 * solely in its dependency blocks. Anything the base cannot be read for is
 * treated as NOT exempt, so the policy fails closed.
 *
 * @param {string[]} files changed paths
 * @returns {boolean}
 */
function isPureDependencyBump(files) {
  if (files.length === 0 || !files.every((file) => DEPENDENCY_ONLY_PATHS.has(file))) {
    return false;
  }
  ...
  let after;
  try {
    before = JSON.parse(git(["show", `${base}:package.json`]));
    after = JSON.parse(fs.readFileSync("package.json", "utf8"));
  } catch {
    return false;
  }

  for (const key of DEPENDENCY_MANIFEST_KEYS) {
    delete before[key];
    delete after[key];
  }
...
function main() {
  ...
  if (isPureDependencyBump(files)) {
    console.log(
      "Changelog check passed: dependency-only change " +
        "(semantic-release records it from the commit message)."
    );
    return;
  }
