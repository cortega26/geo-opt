...
| [090](archive/090-render-python-audits-without-rescoring.md) | Render stored Python reports instead of rereading/rescoring | perf/architecture | P3 | M | 084 | DONE (2026-08-10; executed in worktree — pure `_render_text_report` consumes batch reports, CLI text mode scores each file once (call-count regression, proven to fail pre-fix), text byte-identical to `audit_file` across 10 fixtures; direct `audit_file`, JSON/summary/threshold unchanged; 74 Python tests green) |
