...
export function loadConfig(configPath = null) {
  ...
  if (configPath) {
    ...
    searchPaths.push(path.resolve(__dirname, "..", ".agents", "skills", "geo-optimization", "geo_config.json"));
  }
...
export function auditFile(filepath, config, outputFormat = "text") {
  ...
  if (introPara) {
    ...
    const isDefinition = [" is a ", " is an ", " refers to ", " represents ", " is the strategic "].some(verb => 
      introPara.toLowerCase().includes(verb)
    );
    
    if (wordCount >= 40 && wordCount <= 90) {
      if (isDefinition) {
        structScore += 10;
        structBreakdown.push("Answer-First: Optimal length (40-90 words) and contains definition markers (+10 pts)");
      } else {
        structScore += 7;
        structBreakdown.push("Answer-First: Optimal length but lacks clear definition markers (+7 pts)");
      }
    } else {
      structBreakdown.push(`Answer-First: Intro paragraph has ${wordCount} words (optimal is 40-90) (+0 pts)`);
    }
  } else {
    structBreakdown.push("Answer-First: No intro paragraph found (+0 pts)");
  }
  
  if ((textContent.includes("|") && /\|\s*:?-+:?\s*\|/.test(textContent)) || textContent.toLowerCase().includes("<table>")) {
    structScore += 4;
  ...
  if (filepath.endsWith('.html') || content.toLowerCase().includes("<html")) {
    ...
    if (foundTags.length >= 3) {
      structBreakdown.push(`Semantic HTML: Good HTML5 layout tags used (${foundTags.join(', ')}) (+0 pts)`);
    } else {
      const deduction = 4;
      structScore = Math.max(0, structScore - deduction);
      structBreakdown.push(`Semantic HTML: Lacks HTML5 structural tags (e.g. <main>, <article>). Found only: ${foundTags.join(', ')} (-${deduction} pts)`);
    }
    ...
    if (foundDynamic.length > 0) {
      structBreakdown.push("Dynamic Rendering Warning: Detects client-side JS references. Ensure content is pre-rendered / SSR for AI crawler searchability.");
    }
  }
  ...
  const statMatches = textContent.match(/\b\d+(?:\.\d+)?%|\$\d+(?:\.\d+)?[kKmMbB]?|\b\d+(?:\.\d+)?[xX]\b|\b\d{2,}(?:,\d{3})*(?:\.\d+)?\b/g) || [];
  ...
  if (totalWordCount > 0) {
    ...
    if (pronounDensity > pronounLimit) {
      ...
      clarityBreakdown.push(`Pronoun Ambiguity: High density of ambiguous pronouns (${(pronounDensity * 100).toFixed(1)}%). Limit use of 'it', 'they', etc. (-${deduction} pts)`);
    } else {
      clarityBreakdown.push(`Pronoun Ambiguity: Low density of ambiguous pronouns (${(pronounDensity * 100).toFixed(1)}%) (+0 pts)`);
    }
    ...
    const stopwords = new Set(["THE", "AND", "FOR", "BUT", "YOU", "NOT", "YES", "OUT", "OFF", "HOW", "WHY", "OUR", "WHO"]);
    ...
    if (unexplained.length > 0) {
      ...
      clarityBreakdown.push(`Acronym Clarity: Unexplained acronyms found: ${unexplained.join(', ')}. Spell them out on first mention (-${deductPts} pts)`);
    } else {
  ...
  const recs = [];
  if (structScore < 15) {
    recs.push("Format the opening paragraph to be a self-contained definition/summary of 40-90 words (Answer-First).");
    recs.push("Use markdown tables, headers, and bulleted lists to break up dense blocks of text.");
  }
  if (statsScore < 20) {
    recs.push("Add specific metrics, percentages, dollar values, or dates from studies or reports to support your claims.");
  }
  if (quotesScore < 20) {
    recs.push("Include direct quotes from experts or industry leaders to increase authority.");
  }
  if (citationScore < 20) {
    recs.push("Add external hyperlinks to reputable sources and include a 'References' or 'Sources' list.");
  }
  if (clarityScore < 18) {
    recs.push("Replace ambiguous pronouns ('it', 'they', 'this') with specific nouns (e.g. 'the database', 'this setup').");
    recs.push("Spell out acronyms when they are first used (e.g., 'SaaS (Software as a Service)').");
  }
...
export function checkRobots(robotsPath) {
  ...
  for (let line of lines) {
    ...
    if (disallowMatch && currentAgent) {
      ...
      if (disallowedPath === "/" || disallowedPath === "/*" || (currentAgent === "*" && (disallowedPath === "/" || disallowedPath === "/*"))) {
...
export function injectSchema(filepath, schemaType, config) {
  ...
  const scriptPattern = /<script[^>]*type="application\/ld\+json"[^>]*>[\s\S]*?https:\/\/schema\.org[\s\S]*?<\/script>/i;
