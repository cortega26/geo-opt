...
- **perf (Plan 088):** repeated-stat attribution scanning is linear — `observeAttributionProximity` captures every statistic occurrence's index in one regex iteration and reuses it for year/technical-ID filtering and source-window evaluation, and resolves quotes through one indexOf pass per distinct needle, instead of re-searching the whole text with `nthIndexOf` per occurrence; every count, window, and v2 corpus score is preserved (adversarial audit 2026-08-10: A/B-verified byte-identical output vs the pre-refactor code on 33 markdown/HTML cases incl. interleaved values, contextual-ID slot consumption, boundary-clipped source patterns, metachar quotes and \r\n text; 11 focused tests incl. a scan-work counter regression proving O(matches + text) scaling on repeated values and a window-boundary clip pin)
