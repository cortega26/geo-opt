...
- **perf (Plan 089):** each v2 document is prepared exactly once per call — a new internal `prepareDocument` value (raw content, detector-view text, marked tokens, HTML metadata, v1-view preprocessed text) is built once and shared by profile detection, observations, and clarity scoring in `scoreContentV2`, and `observeAndParse` no longer re-parses through `observeContent`; public `observeContent`/`observeAndParse` signatures are unchanged and every report/fixture score is byte-identical (A/B-verified on 33 corpus fixtures × 2 configs and HTML/malformed inputs); 12 new tests pin parse counts (preprocess/HTML-extract/lex each once per call, no cross-call caching, opts passthrough and public-path agreement) via injectable counters
