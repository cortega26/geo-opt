...
- **tooling (audit 2026-08-10):** the test-count badge gate no longer re-runs the full JS suite — `npm run check` and CI capture the `npm test` output into a log and `check-test-count.js --from-log` reads the count from it; the standalone fallback keeps running the suite but with a 300s timeout, a 256MB buffer, and a clear report of the kill signal instead of a baffling "could not parse test count". The old pattern ran the whole suite twice back-to-back, which got killed on memory-constrained CI runners — red-lighting the badge gate (and every dependabot PR) while the badge was actually correct
