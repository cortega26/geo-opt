...
- Technical audit no longer flags `WebSite`/`Organization` descriptions
  that paraphrase visible branding instead of quoting it verbatim
  (site-tagline false positive observed across whole-site audits).
...
* **v2:** reuse prepared document state (Plan 089) ([b1ca805](https://github.com/cortega26/geo-opt/commit/b1ca805e7c742c05b94180a249bde94548216139))
...
- **docs (Plan 091):** v2 is consistently documented as the default scoring model — `documentation-governance.md` and `architecture.md` no longer claim v1 is the default or that a v1→v2 switch is pending (the switch happened at roadmap gate T0, 2026-06-27); v1 is uniformly legacy/deprecated but selectable via `--model v1`; new invariant tests pin the v2-default claim, scan current docs for v1-default contradictions, and keep Python documented as v1-only and v2 as experimental/non-predictive
- **perf (Plan 090):** Python CLI text-mode auditing renders the reports already produced by the batch pass instead of re-reading and re-scoring each file — a pure `_render_text_report(report, filepath)` consumes the stored report (byte-identical to `audit_file`'s text output, verified across 10 fixtures), each file is scored at most once per audit (call-count regression pins one scoring pass per file), and a file mutated between scoring and rendering no longer affects the output; direct `audit_file` callers and JSON/summary/threshold behavior are unchanged
- **Tooling (Plan 087):** new canonical `npm run test:verify` runs the whole JS suite under c8 **once** and verifies both the test-count and branch-coverage README badges from that same run (count equality + coverage floor, suite exit status preserved, output streamed live); `npm run check` and both CI matrix legs now use it, collapsing the previous test/tee, count-check, coverage, and coverage-check steps into one suite run — the coverage badge is now enforced locally too; `scripts/check-test-count.js` and `scripts/check-coverage.js` are superseded by `scripts/verify-badges.js` (importable parsers, injectable spawn seam). Adversarial audit (2026-08-10): the gate now resolves only after the run log has fully flushed to disk (the old resolve-on-`close` could read a truncated log and red-light on a successful suite — deterministic regression test pins it via an injectable log stream), unparseable run output fails cleanly with the log path instead of an unhandled rejection, a missing local c8 binary reports an actionable `npm ci` hint, pipe errors are swallowed, and temp logs are cleaned up; 15 focused unit tests cover parsers (TAP/spec), badge drift, raised-coverage floor, failing-suite propagation, malformed output, flush-wait and the exactly-once invocation counter
...
- **tooling (audit 2026-08-10):** the test-count badge gate no longer re-runs the full JS suite — `npm run check` and CI capture the `npm test` output into a log and `check-test-count.js --from-log` reads the count from it (the standalone fallback keeps running the suite, hardened with a 300s timeout, a 256MB buffer, and a clear report of kill signals). The gate was also color-blind: Node's test runner prints the summary as `ℹ tests N` on a TTY but as TAP's `# tests N` when stdout is piped, so every CI run (non-TTY) failed to parse its own output and red-lighted the badge gate — and every dependabot PR — while the badge was correct
- **docs (audit 2026-08-10):** the CI dogfood gate (audit `docs/` with threshold 30, a contract that had silently gone red since the 2026-07-31 remediation rewrote `documentation-governance.md`) is repaired: `documentation-governance.md` is remediated back over the threshold (25→44 — answer-first opening, external references to the Node.js release schedule, Keep a Changelog, SemVer and llmstxt.org, inline citations, acronym expansions) and the archival evidence under `docs/audits/` is excluded from the discoverability gate, matching its Historical-record class in the governance doc itself
- **security (audit 2026-08-10):** with the badge and dogfood gates green again, the security-audit gate became the remaining red step and exposed five high advisories. The direct toolchain copy of brace-expansion (eslint > minimatch) is bumped to 5.0.9, resolving both brace-expansion advisories there (CVE-2026-14257 and its GHSA-rgw5-rvv9-x895 bypass); the copies bundled inside the npm CLI tarball that no npm release resolves — brace-expansion 5.0.7 and ip-address 10.2.0, re-verified against npm 11.19.0 (the maximum in-range for `@semantic-release/npm`'s `^11.6.2`) and 12.0.2 (latest) on 2026-08-10 — are explicitly allowlisted with recheck dates in `scripts/check-audit.js`, which `overrides` cannot reach because bundled dependencies ship as-is. The remaining js-yaml and undici advisories are resolved by the dependabot bumps to 4.3.1 and 7.29.0 respectively
- **copy (Plan 085):** v2 wording no longer predicts engine outcomes — the `solid`, `needs-work`, and `at-risk` readiness-band descriptions now describe observed style markers and specific gaps (no more "raise it to production-ready", "likelihood of being cited", or "unlikely to cite"); the plain-English summary reports strong/moderate/weak style markers instead of "well-optimized for AI discovery" / "reliably discovered by AI engines"; and the bundled skill presents the bands as style-marker bands with their stable JSON ids instead of "Production-Ready". Band ids, thresholds, and scores are unchanged; new invariant tests pin the wording
- **copy (Plan 085 audit):** the plain-English summary branches now mirror the readiness-band boundaries (85/65/45) so the summary line and the band label cannot contradict each other (a Solid-band report no longer says "strong style markers", and the 50–64 range no longer mixes "moderate" with the band's "partial"); boundary pins cover every threshold edge in the summary and `readinessBand`; the CLI sample output in both READMEs shows the current at-risk description; and the accepted-advisories table in `plans/README.md` now documents the two new security-gate allowlist entries (GHSA-rgw5-rvv9-x895, GHSA-mwp4-54f8-5fhr)
